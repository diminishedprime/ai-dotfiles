...
1. Consider whether the _why_ behind the changes is obvious from context. If the
   ...
   prompted a fix - ask a brief clarifying question or two first. Keep it brief:
   one or two focused questions, not an interrogation. The goal is to capture
   the real motivation without being tedious.

   **Use the an Ask Questions tool for these clarifying questions.** Do not pose
   them as a numbered list in plain text - answering a numbered list by hand is
   annoying. The tool gives the user a proper structured prompt.
   ...
   **You are an LLM. You cannot reconstruct user motivation from a diff.** The
   ...
   stop and ask the user instead of inventing one. A diff feeling self-evident
   is not the same as you actually knowing why the user made it.
...
2. Craft a commit message. Format: `<type>[(<scope>)]: <subject>` with an
   ...
   Subject: imperative mood, no trailing period, < 73 characters.
...
3. **Default to a body that explains the _why_.** The commit message is
   ...
   **Common failure modes to avoid** (you will be tempted):
   ...
   - Inverting the change into a "why" ("we wanted X to happen" for a change
     that makes X happen - the diff already shows that). If the only motivation
     you can articulate is the inverse-restatement of the change, either find a
     deeper why (the original friction, a prior incident, a constraint) or drop
     the body line entirely.
...
4. **Audit before showing.** Invoke the `semantic-commit-audit` subagent (via
   ...
   auditor until it returns `APPROVED`** - every revised draft requires a fresh
   audit call, even if the change feels minor or you're confident the fix
   addresses the violation. Reasoning "I trimmed the offending sentence, so it
   must pass now" is not a substitute for actually running the audit. Do not
   show the user any draft that has not, on its most recent revision, come back
   `APPROVED`.

   **If the auditor flags "narration" or "doesn't explain why" on three
   consecutive revisions, stop iterating and go ask the user.** That pattern
   means you are guessing at motivation rather than reporting it, and each new
   draft is just substituting one plausible fabrication for another. The fix is
   to get the why from the user via an Ask Questions tool (same reasoning as
   step 1 - one question per change whose motivation you're missing, with
   concrete candidate motivations as options) and draft a new revision from
   their words. Then re-audit that revision like any other.

   **The auditor is not optional and you do not get to overrule it by appealing
   to the user.** Showing the user an un-`APPROVED` draft alongside the
   auditor's outstanding complaints — inviting them to wave it through, sharpen
   it for you, or confirm the auditor is being too strict — is a way of
   laundering an audit failure into approval. Don't do it. If you are stuck, the
   next move is always to gather more grounding from the user (their motivation,
   in their words) and feed it back into a new revision that you then re-audit.
   The user only sees drafts that have come back `APPROVED` on their most recent
   revision.

5. Present the (audited) message to the user. Ask if they'd like to commit or
   adjust it.

6. If the user wants changes, iterate — re-audit each new draft — until they
   approve. On approval, run `git commit` using a HEREDOC for the message. **Run
   the commit in the background** (`run_in_background: true`). Pre-commit hooks
   routinely take long enough that a foreground call looks hung and derails the
   session; backgrounding lets the harness notify you when it finishes. Once
   notified, check the result and report success or surface the hook failure to
   the user.
